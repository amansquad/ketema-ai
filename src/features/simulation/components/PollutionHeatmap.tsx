...
function buildMarkers(objects: SceneObject[]): HeatmapMarker[] {
  return objects
    .map((object) => ({ object, weight: pollutionWeightFor(object.assetKind) }))
...
// buildings, hospitals) — a lightweight stand-in for a full density-field
// heatmap, toggled from the simulation control panel.
export function PollutionHeatmap() {
  ...
  const markers = useMemo(() => buildMarkers(objects), [objects]);
